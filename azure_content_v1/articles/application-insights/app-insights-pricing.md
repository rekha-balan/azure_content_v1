...
* [Monthly Quota](#monthly-quota.md) - the amount of telemetry you can analyze each month.
* [Data rate](#data-rate.md) - the maximum rate at which data from your app can be processed.
* [Retention](#data-retention.md) - how long data is kept in  the Application Insights portal for you to view.
* [Continuous export](#continuous-export.md) - whether you can export data to other tools and services.
...
* Data points are generated by:  * [SDK modules](app-insights-configuration-with-applicationinsights-config.md) that automatically collect data, for example to report a request or crash, or to measure performance.
...
* If you'd like to count up data points by inspection, you can find them in various places:  * Each item you see in [diagnostic search](app-insights-diagnostic-search.md), which includes HTTP requests, exceptions, log traces, page views, dependency events, and custom events.
...
* You can also inspect individual data points at source during debugging:  * If you run your app in debug mode in Visual Studio, the data points are logged in the Output window. 
* To see client data points, open your browser's debugging pane (usually F12) and open the network tab.


* The data rate is (by default) reduced by [adaptive sampling](app-insights-sampling.md). This means that, as usage of your app increases, the telemetry rate won't increase as much as you might expect.
...
### Name limits
1. Maximum of 200 unique metric names and 200 unique property names for your application. Metrics include data sent via TrackMetric as well as measurements on other  data types such as Events.  [Metrics and property names](app-insights-api-custom-events-metrics.md) are global per instrumentation key not scoped to data type.
2. [Properties](app-insights-api-custom-events-metrics.md#properties) can be used for filtering and group by only while they have less than 100 unique values for each property. After the unique values exceed 100, the property can still be used for search and filtering but no longer for filters.
