...
[extension for an Azure website or VM](../insights-perf-analytics.md).
...
### Dependency Tracking
[Dependency tracking](app-insights-dependencies.md) collects telemetry about calls your app makes to databases and external services and databases. To allow this module to work in an IIS server, you need to [install Status Monitor](app-insights-monitor-performance-live-website-now.md). To use it in Azure web apps or VMs, [select the Application Insights extension](../insights-perf-analytics.md).
...
tracked by this module appears in the [Diagnostic Search](app-insights-diagnostic-search.md).
...
* `DeviceTelemetryInitializer` updates the following properties of the `Device` context for all telemetry items.  * `Type` is set to "PC"
