...
This topic describes [CoreOS](https://coreos.com/) and shows how to create a cluster of three CoreOS virtual machines on Azure as a quick start to understanding this operating system. It uses the very basic elements of CoreOS deployments and examples from [CoreOS with Azure](https://coreos.com/docs/running-coreos/cloud-providers/azure/), [Tim Park's CoreOS Tutorial](https://github.com/timfpark/coreos-azure), and [Patrick Chanezon's CoreOS Tutorial](https://github.com/chanezon/azure-linux/tree/master/coreos/cloud-init) to demonstrate the absolute minimum requirements to both understand the basic structure of a CoreOS deployment and get a cluster of three virtual machines running successfully.
...
CoreOS is a lightweight version of Linux designed to support rapid creation of potentially very large clusters of VMs that use Linux containers as the only packaging mechanism, including [Docker](http://docker.io) containers. CoreOS is intended to support:
...
This is a very general description of CoreOS and its features. For more complete information about CoreOS, see the [CoreOS Overview](https://coreos.com/using-coreos/).
...
## How to use CoreOS on Azure
This section describes how to create an Azure cloud cervice with three CoreOS virtual machines in it using the [Azure Command-line Interface (Azure CLI)](../xplat-cli-install.md). The basic steps are as follows:
...
3. Create a cloud-config file in [YAML](http://yaml.org/) format
...
> [!NOTE]
> This topic assumes that you do not have these keys, and requires you to create `myPrivateKey.pem` and `myCert.pem` files for clarity. If you already have a public and private key pair saved to `~/.ssh/id_rsa`, you can just type `openssl req -x509 -key ~/.ssh/id_rsa -nodes -days 365 -newkey rsa:2048 -out myCert.pem` to obtain the .pem file that you need to upload to Azure.
> 
> 
...
1. Install the [Azure Command-line Interface (Azure CLI)](../xplat-cli-install.md) if you have not already done so, and either login using a work or school ID, or download a .publishsettings file and import that into your account.
...
> [!NOTE]
> If you are already using the `~/.ssh/id_rsa` key, then add that with `ssh-add ~/.ssh/id_rsa`.
> 
> 
...
## Next steps
You should now have a running three-node CoreOS cluster on Azure. From here, you can explore how to create more complex clusters and use Docker and create more interesting applications. To try a couple of quick examples, see [Get Started with Fleet on CoreOS on Azure](virtual-machines-linux-coreos-fleet-get-started.md).
