...
To understand how [Azure IoT Suite](iot-suite-overview.md) provides the capabilities customers need to realize the potential of predictive maintenance, please review this [infographic](https://www.microsoft.com/en-us/server-cloud/predictivemaintenance/Index.html).

To show these capabilities working from device telemetry collected through IoT Suite services, and surfaced to users in a web application, the solution leverages an existing Azure Machine Learning model available as a template. Microsoft has built a [regression model](http://gallery.cortanaanalytics.com/Collection/Predictive-Maintenance-Template-3) and published the complete template, data<sup>\[1\]</sup>, and step-by-step guidance.
