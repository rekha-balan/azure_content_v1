...
* **SELECT**: Selects a count of all rows where the column **t4** contains the value **[ERROR]ERROR]**. This should return a value of **3** because there are three rows that contain this value.
...
* **INSERT OVERWRITE ... SELECT**: Selects rows from the **log4jLogs** table that contains **[ERROR]ERROR]**, and then inserts the data into the **errorLogs** table.
