...
* **SELECT**: Selects a count of all rows where column **t4** contains the value **[ERROR]ERROR]**. This should return a value of **3** because there are three rows that contain this value.
...
1. Use the following statements to create a new 'internal' table named **errorLogs**:
...
* **INSERT OVERWRITE ... SELECT**: Selects rows from the **log4jLogs** table that contain **[ERROR]ERROR]**, then inserts the data into the **errorLogs** table.
  ...
  To verify that only rows that contain **[ERROR]ERROR]** in column t4 were stored to the **errorLogs** table, use the following statement to return all the rows from **errorLogs**:
