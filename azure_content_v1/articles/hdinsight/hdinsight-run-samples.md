...
* [**Word count**](#hdinsight-sample-wordcount.md): Counts word occurrences in a text file.
* [**C# streaming word count**](#hdinsight-sample-csharp-streaming.md): Counts word occurrences in a text file using the Hadoop streaming interface.
* [**Pi estimator**](#hdinsight-sample-pi-estimator.md): Uses a statistical (quasi-Monte Carlo) method to estimate the value of pi.
* [**10-GB Graysort**](#hdinsight-sample-10gb-graysort.md): Run a general purpose GraySort on a 10 GB file by using HDInsight. There are three jobs to run: Teragen to generate the data, Terasort to sort the data, and Teravalidate to confirm that the data has been properly sorted.
...
The source code can be found in the [Appendix A](#apendix-a---the-word-count-MapReduce-program-in-java.md).
...
1. Open **Windows PowerShell ISE**. For instructions, see [Install and configure Azure PowerShell](../install-configure-powershell.md).
...
> 
> 
In the example, the mapper and the reducer are executables that read the input from [stdin][stdin-stdout-stderr]stdin][stdin-stdout-stderr](line-by-line.md) and emit the output to [stdout][stdin-stdout-stderr]stdout][stdin-stdout-stderr]stdin-stdout-stderr]. The program counts all of the words in the text.
...
For more information about the Hadoop Streaming interface, see [Hadoop Streaming][hadoop-streaming]Hadoop Streaming][hadoop-streaming]hadoop-streaming].

**To submit a C# streaming word count job**

* Follow the procdure in [Word count - Java](#word-count-java.md), and replace the job definition with the following:
...
* Follow the procdure in [Word count - Java](#word-count-java.md), and use the following job definitions:
...
* [Get started using Hadoop with Hive in HDInsight to analyze mobile handset use](../hdinsight-get-started.md)
