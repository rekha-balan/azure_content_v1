...
* To begin, open the `config.js` file in the root of the project, and enter your app's configuration values in the `exports.creds` section.  * The `clientID:` is the **Application Id** assigned to your app in the registration portal.
...
* Let's review these in detail:  * The `/` route will redirect to the index.ejs view passing the user in the request (if it exists)
