...
* **An on-premises management server**: The management server runs Site Recovery components:  * **Configuration server**:  Coordinates communication and manages data replication and recovery processes.
...
[Follow these instructions](#deploy-additional-process-servers.md) to set up an additional process server.
...
> 
> 
[AZURE.VIDEO enhanced-vmware-to-azure-setup-registration]AZURE.VIDEO enhanced-vmware-to-azure-setup-registration]
...
> [AZURE.VIDEO enhanced-vmware-to-azure-discovery]AZURE.VIDEO enhanced-vmware-to-azure-discovery]
> 
> 
...
1. On the vCenter server create a role (**Azure_Site_Recovery**) at the vCenter level with the [required permissions](#vmware-permissions-for-vcenter-access.md).
...
> 
3. To add the account open **cspsconfigtool**. It's available as a shortcut on the desktop and located in the [INSTALL LOCATION]INSTALL LOCATION]\home\svsystems\bin folder.
...
> [AZURE.VIDEO enhanced-vmware-to-azure-protection]AZURE.VIDEO enhanced-vmware-to-azure-protection]
> 
> 
...
3. Add the account you created:
   ...
   * Open **cspsconfigtool**. It's available as a shortcut on the desktop and located in the [INSTALL LOCATION]INSTALL LOCATION]\home\svsystems\bin folder.
...
1. Make sure that the Linux machine you want to protect is supported as described in [On-premises prerequisites](#on-premises-prerequisites.md). Ensure there’s network connectivity between the machine you want to protect and the management server that runs the process server. 
...
UnifiedAgent.exe [/Role <Agent/MasterTarget>] [/InstallLocation <Installation Directory>]/Role <Agent/MasterTarget>] [/InstallLocation <Installation Directory>] [/CSIP <IP address of CS to be registered with>]/InstallLocation <Installation Directory>] [/CSIP <IP address of CS to be registered with>] [/PassphraseFilePath <Passphrase file path>]/CSIP <IP address of CS to be registered with>] [/PassphraseFilePath <Passphrase file path>] [/LogFilePath <Log File Path>]/PassphraseFilePath <Passphrase file path>] [/LogFilePath <Log File Path>]/LogFilePath <Log File Path>] 
...
5. In **Specify Accounts** select the account you [configured](#install-the-mobility-service-with-the-process-server.md) to use for automatic installation of the Mobility service. 
...
* **Azure VM size**: The number of network adapters is dictated by the size you specify for the target virtual machine. [Read more](virtual-machines-size-specs.md/#size-tables.md) about sizes and adapters. Note that:  * When you modify the size for a virtual machine and save the settings, the number of network adapter will change when you open the **Configure** tab next time. The number of network adapters of target virtual machines is minimum of the number of network adapters on source virtual machine and maximum number of network adapters supported by the size of the virtual machine chosen.   * If the number of network adapters on the source machine is less than or equal to the number of adapters allowed for the target machine size, then the target will have the same number of adapters as the source.
...
> [AZURE.VIDEO enhanced-vmware-to-azure-failover]AZURE.VIDEO enhanced-vmware-to-azure-failover]
> 
> 
...
3. Complete the wizard in the same way you did when you [set up](#step-5:-install-the-management-server.md) the first management server. 
