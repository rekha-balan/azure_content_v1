...
The cmdlet creates a Traffic Manager profile in Azure Traffic Manager and returns a corresponding profile object.  At this point, the profile does not contain any endpoints—see [Adding Traffic Manager Endpoints](#adding-traffic-manager-endpoints.md) for details of how to add endpoints to a Traffic Manager profile.
...
## Update a Traffic Manager Profile [](#update-traffic-manager-profile.md)
...
1. Using a 3-step process similar to that described in [Update a Traffic Manager Profile](#update-traffic-manager-profile.md): get the profile object using Get-AzureRmTrafficManagerProfile; update it off-line to add an endpoint, using Add-AzureRmTrafficManagerEndpointConfig; upload changes to Azure Traffic Manager using Set-AzureRmTrafficManagerProfile.  The advantage of this method is that a number of endpoint changes can be made in a single update.
