...
* [Reporting aggregation functions](#reporting-aggregation-functions.md), such as SUM or AVG
* [Ranking functions](#ranking-functions.md), such as DENSE_RANK, ROW_NUMBER, NTILE, and RANK
* [Analytic functions](#analytic-functions.md),  such as cumulative distribution, percentiles, or accesses data from a previous row in the same result set without the use of a self-join
...
* 100 rows divided into 4 groups: [ 25, 25, 25, 25 ] 25, 25, 25, 25 ]
* 102 rows devided into 4 groups: [ 26, 26, 25, 25 ] 26, 26, 25, 25 ]
...
WITHIN GROUP ( ORDER BY <identifier> [ ASC | DESC ] ASC | DESC ]) - Specifies a list of numeric values to sort and compute the percentile over. Only one column identifier is allowed. The expression must evaluate to a numeric type. Other data types are not allowed. The default sort order is ascending.

OVER ([ PARTITION BY <identifier,>…[n] ] PARTITION BY <identifier,>…[n]n] ] ) - Divides the input rowset into partitions as per the partition key to which the percentile function is applied. For more information, see RANKING section of this document.
