...
> [!NOTE]
> You might need to **Run as Administrator** when executing the following PowerShell script if your *DestDir* needs Administrator privilege to create or to write to. 
> 
> 
...
* Load data from your private blob storage account to your Azure SQL DW  * Create external tables for NYC taxi dataset on the blob storage account
...
> [!NOTE]
> In order to avoid schema name conflicts with those that already exist in your Azure SQL DW, when reading parameters directly from the .conf file, a 3-digit random number is added to the schema name from the .conf file as the default schema name for each run.  
> 
> [!NOTE]
...
2. Persist the sampled and engineered data you plan to use for model building in a new SQL DW table and use the new table in the [Reader](https://msdn.microsoft.com/library/azure/4e1b0fe6-aded-4b3f-a36f-39b8862b9004/) module in Azure Machine Learning. The PowerShell script in earlier step has done this for you. You can read directly from this table in the Reader module. 
