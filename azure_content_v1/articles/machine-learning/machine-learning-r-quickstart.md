...
If you need a more complete treatment of R exception handling, I recommend you read the applicable sections of the book by Wickham listed in [Appendix B - Further Reading](#appendixb.md).
...
I provide some additional information on using RStudio in [Appendix A](#appendixa.md).  
...
> [!NOTE]
> Azure Machine Learning treats files in the zip as if they are in the src/ directory, so you need to prefix your file names with this directory name.  
> 
> 
We already discussed loading datasets in [Loading the dataset](#loading.md). Once you have created and tested the R script shown in the previous section, do the following:
...
R dataframes support powerful filtering capabilities. Datasets can be subsetted by using logical filters on either rows or columns. In many cases, complex filter criteria will be required. The references in [Appendix B - Further reading](#appendixb.md) contain extensive examples of filtering dataframes.  
...
In this section we will perform some simple transformations on the values in some of the columns of our dataframe. The R language supports nearly arbitrary value transformations. The references in [Appendix B - Further Reading](#appendixb.md) contain extensive examples.
...
We have already discussed an example of defensive programming in [Value transformations](#valuetransformations.md). Both computation blocks are wrapped in `tryCatch()`. For some errors it makes sense to return the original input vector, and in other cases, I return a vector of zeros.  
