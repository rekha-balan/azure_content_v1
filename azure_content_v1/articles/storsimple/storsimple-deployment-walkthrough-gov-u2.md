...
| [Deployment configuration checklist](#deployment-configuration-checklist.md) |Use this checklist to gather and record information prior to and during the deployment. |
| [Deployment prerequisites](#deployment-prerequsites.md) |These  validate that the environment is ready for deployment. |
|  | |
| **STEP-BY-STEP DEPLOYMENT** |These steps are required to deploy your StorSimple device in production. |
| [Step 1: Create a new service](#step-1-create-a-new-service.md) |Set up cloud management and storage for your StorSimple device. *Skip this step if you have an existing service for other StorSimple devices*. |
| [Step 2: Get the service registration key](#step-2-get-the-service-registration-key.md) |Use this key to register and connect your StorSimple device with the management service. |
...
| [Step 4: Complete the minimum device setup](#step-4-complete-the-minimum-device-setup.md) </br>Optional: Update your StorSimple device. |Use the management service to complete the device setup and enable it to provide storage. |
| [Step 5: Create a volume container](#step-5-create-a-volume-container.md) |Create a container to provision volumes. A volume container has storage   account, bandwidth, and encryption settings for all the volumes contained in it. |
| [Step 6: Create a volume](#step-6-create-a-volume.md) |Provision storage volume(s) on the StorSimple device for your servers. |
| [Step 7: Mount, initialize, and format a volume](#step-7-mount-initialize-and-format-a-volume.md) </br>Optional: Configure MPIO. |Connect your servers to the iSCSI storage provided by the device. Optionally, configure MPIO to ensure that your servers can tolerate link, network, and interface failure. |
| [Step 8: Take a backup](#step-8-take-a-backup.md) |Set up your backup policy to protect your data |
|  | |
| **OTHER PROCEDURES** |You may need to refer to these procedures as you deploy your solution. |
| [Configure a new storage account for the service](#configure-a-new-storage-account-for-the-service.md) | |
| [Use PuTTY to connect to the device serial console](#use-putty-to-connect-to-the-device-serial-console.md) | |
| [Scan for and apply updates](#scan-for-and-apply-updates.md) | |
| [Get the IQN of a Windows Server host](#get-the-iqn-of-a-windows-server-host.md) | |
| [Create a manual backup](#create-a-manual-backup.md) | |
| [Configure MPIO](#configure-mpio.md) | |
