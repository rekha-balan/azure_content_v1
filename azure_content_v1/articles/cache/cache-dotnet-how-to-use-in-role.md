...
information on using In-Role Cache, refer to the [Next Steps](#next-steps.md) section.
...
* [Configure the cache cluster](#enable-caching.md)
* [Configure the cache clients](#NuGet.md)
...
After the configuration is added, replace **[cache cluster role name]cache cluster role name]** with the name of the role that hosts the cache cluster.

> If **[cache cluster role name]cache cluster role name]** is not replaced with the name of the role that hosts the cache cluster, then a **TargetInvocationException** will be thrown when the cache is accessed with an inner **DatacacheException** with the message "No such role exists".
> 
> 
...
* [How To: Create a DataCache Object](#create-cache-object.md)
* [How To: Add and Retrieve an Object from the Cache](#add-object.md)
* [How To: Specify the Expiration of an Object in the Cache](#specify-expiration.md)
* [How To: Store ASP.NET Session State in the Cache](#store-session.md)
* [How To: Store ASP.NET Page Output Caching in the Cache](#store-page.md)
...
> profile for the project is .NET Framework 4.0 or higher, and be sure to select one of the profiles that does not specify **Client Profile**. For instructions on configuring cache clients, see [Configure the cache clients](#NuGet.md).
> 
> 
...
state provider, first configure your cache cluster, and then configure your ASP.NET application for caching using the Caching NuGet package as described in [Getting Started with In-Role Cache](#getting-started-cache-role-instance.md). When the Caching NuGet package is installed, it adds a commented out section in web.config that contains the required configuration for your ASP.NET application to use the Session State Provider for In-Role Cache.
