...
* **Azure Autoscaling**. This feature supports the most common scaling scenarios based on a schedule and, optionally, triggered scaling operations based on runtime metrics (such as processor utilization, queue length, or built in and custom counters). You can configure simple autoscaling policies for a solution quickly and easily by using the Azure Management Portal. For more fine grained control you can make use of the [Azure Service Management REST API](https://msdn.microsoft.com/library/azure/ee460799.aspx) or the [Azure Resource Manager REST API](https://msdn.microsoft.com//library/azure/dn790568.aspx). The [Azure Monitoring Service Management Library](http://www.nuget.org/packages/Microsoft.WindowsAzure.Management.Monitoring) and the [Microsoft Insights library](https://www.nuget.org/packages/Microsoft.Azure.Insights/) (in preview) are SDK's which allow collecting metrics from different resources and perform autoscaling making use of the REST API's. For resources where Azure Resource Manager support is not available or if you are using Cloud Services the Service Management REST API can be used for autoscaling. It is recommended to use the Azure Resource Manager (ARM) for autoscaling in all other cases.
* **A custom solution** based your instrumentation on the application, and management features of Azure. For example, you could use Azure diagnostics or other methods of instrumentation in your application along with custom code to continually monitor and export metrics of the application. You could have custom rules which work on these metrics which make use of the Service Management or Resource Manager REST API's to trigger autoscaling. The metrics for triggering a scaling operation can be any built-in or custom counter, or other instrumentation you implement within the application. However, a custom solution is not simple to implement, and should be considered only if none of the previous approaches can fulfil your requirements. The [Autoscaling Application Block](http://msdn.microsoft.com/library/hh680892%28v=pandp.50%29.aspx) made use of this approach.
...
* When you configure multiple policies and rules, there is a possibility that they could conflict with each other. Azure Autoscaling uses the following conflict resolution rules to ensure that there is always a sufficient number of instances running:  * Scale out operations always take precedence over scale in operations.
