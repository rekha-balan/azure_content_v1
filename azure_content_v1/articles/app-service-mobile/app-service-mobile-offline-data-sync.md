...
To use the SQLite-based implementation on Windows Phone or Windows Store 8.1, you need to install a SQLite extension. For more details, see [Windows 8.1: Enable offline sync](../app-service-mobile-windows-store-dotnet-get-started-offline-data.md). Android and iOS ship with a version of SQLite in the device operating system itself, so it is not necessary to reference your own version of SQLite.
