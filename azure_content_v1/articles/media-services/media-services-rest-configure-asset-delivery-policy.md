...
For information on what values you can specify when creating an AssetDeliveryPolicy, see the [Types used when defining AssetDeliveryPolicy](#types.md) section.   
...
See [Link asset with asset delivery policy](#link_asset_with_asset_delivery_policy.md)
...
### Get Delivery URL
Get the delivery URL for the PlayReady delivery method of the content key created in the previous step. A client uses the returned URL to request a PlayReady license in order to playback the protected content. For more information, see [Get Delivery URL](#get_delivery_url.md).
