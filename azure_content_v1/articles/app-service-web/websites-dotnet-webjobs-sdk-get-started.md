...
> [!NOTE]
> For the code specific to the WebJobs SDK, go to the [Program.cs and Functions.cs](#programcs.md) sections.
> 
> 
...
Similar code gets a reference to the *blobnamerequest* queue and creates a new queue. In this case no permissions change is needed. The [ResolveBlobName](#resolveblobname.md) section later in the tutorial explains why the queue that the web application writes to is used just for getting blob names and not for generating thumbnails.
