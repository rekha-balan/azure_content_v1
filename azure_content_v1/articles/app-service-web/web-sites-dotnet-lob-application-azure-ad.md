...
* [Azure SDK 2.8.1](http://go.microsoft.com/fwlink/p/?linkid=323510clcid=0x409) or later
...
8. Add the highlighted [Authorize]Authorize] decorations to the respective actions below.
...
10. In Create() and Edit() add following code to make some variables available to your JavaScript later. Right-click on each naming resolution error to fix it.
     ...
     ViewData["token"]"token"] = AcquireToken(ClaimsPrincipal.Current.FindFirst(Globals.ObjectIdClaimType).Value);
 ViewData["tenant"]"tenant"] = ConfigHelper.Tenant;
