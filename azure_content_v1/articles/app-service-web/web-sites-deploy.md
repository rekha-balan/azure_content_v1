...
for continuous deployment and other automation tasks. All these services are customizable and triggered   * every time **git push** is executed from a configured Git repository,
...
specific tools you choose. For example, if you perform Web Deploy from [Visual Studio with Azure SDK](#vspros.md), even though you don't get automation 
...
> [!NOTE]
> These deployment processes don't actually [provision the Azure resources](resource-group-portal.md) that your app may need, such as 
...
> [Automate deployment by using command-line tools](#automate.md) section.
> 
> 
...
Copying files manually utilizes the FTP protocol for deployment (see [Overview of deployment processes](#overview.md)).
...
A good alternative to [copying files manually](#ftp.md) is syncing files and folders to App Service from a cloud storage service
...
Syncing with a cloud folder utilizes the Kudu process for deployment (see [Overview of deployment processes](#overview.md)).
...
deploy to App Service if they have FTP or Git integration (see [Overview of deployment processes](#overview.md)).
...
TFS uses Web Deploy to deploy to App Service, while deployment from Git/Mercurial repositories uses Kudu (see [Overview of deployment processes](#overview.md)).
...
* Branch-specific deployment, can deploy different versions to separate [slots](web-sites-staged-publishing.md).
...
Kudu (see [Overview of deployment processes](#overview.md)).
...
* [Automate deployment with MSBuild](#msbuild.md)
* [Copy files with FTP tools and scripts](#ftp.md)
* [Automate deployment with Windows PowerShell](#powershell.md)
* [Automate deployment with .NET management API](#api.md)
* [Deploy from Azure Command-Line Interface (Azure CLI)](#cli.md)
* [Deploy from Web Deploy command line](#webdeploy.md)
* [Using FTP Batch Scripts](http://support.microsoft.com/kb/96269).
...
If you use the [Visual Studio IDE](#vs.md) for development, you can use [MSBuild](http://msbuildbook.com/) to automate anything you can do in your IDE. You can configure MSBuild to use either [Web Deploy](#webdeploy.md) or [FTP/FTPS](#ftp.md) to copy files. Web Deploy can also automate many other deployment-related tasks, such as deploying databases.
