...
**Explanation**:
Use LAST function to retrieve last Time value when event type was ‘Start’. Note that LAST function uses PARTITION BY [user]user] to indicate that result shall be computed per unique user.  The query has a 1 hour maximum threshold for time difference between ‘Start’ and ‘Stop’ events but is configurable as needed (LIMIT DURATION(hour, 1).
