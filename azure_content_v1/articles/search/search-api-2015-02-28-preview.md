...
* `moreLikeThis` is a a query parameter used in [Search operations](#SearchDocs.md) that finds other documents that are relevant to another specific document.
...
[Update Index](#UpdateIndex.md)

    PUT /indexes/[index name]?api-version=2015-02-28-Preview

[Get Index](#GetIndex.md)

    GET /indexes/[index name]?api-version=2015-02-28-Preview

[Listing Indexes](#ListIndexes.md)

    GET /indexes?api-version=2015-02-28-Preview

[Get Index Statistics](#GetIndexStats.md)

    GET /indexes/[index name]/stats?api-version=2015-02-28-Preview

[Delete an Index](#DeleteIndex.md)

    DELETE /indexes/[index name]?api-version=2015-02-28-Preview

[Add, Delete, and Update Data within an Index](#AddOrUpdateDocuments.md)

    POST /indexes/[index name]/docs/index?api-version=2015-02-28-Preview

[Search Documents](#SearchDocs.md)

    GET /indexes/[index name]/docs?[query parameters]
    POST /indexes/[index name]/docs/search?api-version=2015-02-28-Preview

[Lookup Document](#LookupAPI.md)

     GET /indexes/[index name]/docs/[key]?[query parameters]

[Count Documents](#CountDocs.md)
...
After the index is created, you'll upload documents that populate the index. See [Add or Update Documents](#AddOrUpdateDocuments.md) for this next step.
...
`type` - Sets the data type for the field. See [Supported Data Types](#DataTypes.md) for a list of supported types.
...
`suggestions` - Previous versions of the API included a `suggestions` property. This boolean property is now deprecated and no longer available in either `2015-02-28` or `2015-02-28-Preview`. Please use the [Suggesters API](#Suggesters.md) instead. In the `2014-07-31` version, the `suggestions` property was used to specify whether the field could be used for auto-complete for type ahead, for fields of type `Edm.String` or `Collection(Edm.String)`. The `suggestions` was `false` by default because it required extra space in your index, but if you enabled it, see [Transition from Preview to General Release in Azure Search](search-transition-from-preview.md) for instructions on how to transition to the new API.

`key` - Marks the field as containing unique identifiers for documents within the index. Exactly one field must be chosen as the `key` field and it must be of type `Edm.String`. Key fields can be used to look up documents directly via the [Lookup API](#LookupAPI.md).
...
`suggesters` - Sets the search mode and fields that are the source of the content for suggestions. See [Suggesters](#Suggesters.md) for details.
...
A `suggester` defines which fields in an index are used to support auto-complete in searches. Typically partial search strings are sent to the [Suggestions API](#Suggestions.md) while the user is typing a search query, and the API returns a set of suggested phrases. A suggester that you define in the index determines which fields are used to build the type-ahead search terms. See [Suggesters](#Suggesters.md) for configuration details.
...
* Invoke suggestions by calling the [Suggestions API](#Suggestions.md) in your application code. Typically partial search strings are sent to the service while the user is typing a search query, and this API returns a set of suggested phrases.


This article explains how to configure a **suggester**. You should also review the [Suggestions API](#Suggestions.md) for details on how a suggester is used.
...
The schema syntax used to create an index is reproduced here for convenience. See [Create Index](#CreateIndex.md) for more details.
...
The [index name]index name] in the request URI specifies which index to return from the indexes collection.
...
See the example JSON in [Creating and Updating an Index](#CreateUpdateIndexExample.md) for an example of the response payload.

<a name="DeleteIndex"></a>

## Delete Index
The **Delete Index** operation removes an index and associated documents from your Azure Search service. You can get the index name from the service dashboard in the Azure portal, or from the API. See [List Indexes](#ListIndexes.md) for details.
...
The [index name]index name] in the request URI specifies which index to delete from the indexes collection.
...
The [index name]index name] in the request URI tells the service to return index statistics for the specified index.
...
Before you can upload documents, you must have already created the index on the service. See [Create Index](#CreateIndex.md) for details about this first step.
...
The request URI includes [index name]index name], specifying which index to post documents. You can only post documents to one index at a time.
...
* `count` (max number of facet terms; default is 10). There is no maximum, but higher values incur a corresponding performance penalty, especially if the faceted field contains a large number of unique terms.  * For example: `facet=category,count:5` gets the top five categories in facet results.  
...
* `values` (pipe-delimited numeric or `Edm.DateTimeOffset` values specifying a dynamic set of facet entry values)  * For example: `facet=baseRate,values:10|20` produces three buckets: One for base rate 0 up to but not including 10, one for 10 up to but not including 20, and one for 20 or higher.
...
* `interval` (integer interval greater than 0 for numbers, or `minute`, `hour`, `day`, `week`, `month`, `quarter`, `year` for date time values)  * For example: `facet=baseRate,interval:100` produces buckets based on base rate ranges of size 100. For example, if base rates are all between $60 and $600, there will be buckets for 0-100, 100-200, 200-300, 300-400, 400-500, and 500-600.
...
> 
> 
`$filter=[string]` (optional) - A structured search expression in standard OData syntax. See [OData Expression Syntax](#ODataExpressionSyntax.md) for details on the subset of the OData expression grammar that Azure Search supports.
...
> [!NOTE]
> When calling **Search** using POST, this parameter is named `scoringParameters` instead of `scoringParameter`. Also, you specify it as a JSON array of strings where each string is a separate name:value pair.
> 
> 
...
The request URI includes an [index name]index name] and [key]key], specifying which document to retrieve from which index. You can only get one document at a time. Use **Search** to get multiple documents in a single request.
...
The [index name]index name] in the request URI tells the service to return a count of all items in the docs collection of the specified index.
...
> 
> 
`suggesterName=[string]` - the name of the suggester as specified in the `suggesters` collection that's part of the index definition. A `suggester` determines which fields are scanned for suggested query terms. See [Suggesters](#Suggesters.md) for details.
