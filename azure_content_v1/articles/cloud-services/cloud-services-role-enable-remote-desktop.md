...
1. In **Roles**, select the role you want to update or select **All** for all roles.
...
1. Expand the **Azure\\Cloud Services\\[cloud service name]cloud service name]** node.
...
```
The [ServiceConfiguration.cscfg](cloud-services-model-and-package.md#cscfg) file should be similar to the following example, note the `<ConfigurationSettings>` and `<Certificates>` elements. The Certificate specified must be [uploaded to the cloud service](cloud-services-how-to-create-deploy/#how-to-upload-a-certificate-for-a-cloud-service.md).
