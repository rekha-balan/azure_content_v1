...
2. Browse to **Active Directory > Directory > [Your Directory]Your Directory] > Applications**, and select **Add > Add an application from the gallery**.
...
2. In the dialog, enter the URL of the application's SCIM endpoin.  
...
4. If the attempts to connect to the application succeed, then click **Next** on the remaining screens, and click **Complete** to exit the dialog.
...
| Facsimile-TelephoneNumber |phoneNumbers[type eq "fax"]type eq "fax"].value |
| givenName |name.givenName |
| jobTitle |title |
| mail |emails[type eq "work"]type eq "work"].value |
| mailNickname |externalId |
| manager |manager |
| mobile |phoneNumbers[type eq "mobile"]type eq "mobile"].value |
| objectId |id |
| postalCode |addresses[type eq "work"]type eq "work"].postalCode |
| proxy-Addresses |emails[type eq "other"]type eq "other"].Value |
| physical-Delivery-OfficeName |addresses[type eq "other"]type eq "other"].Formatted |
| streetAddress |addresses[type eq "work"]type eq "work"].streetAddress |
| surname |name.familyName |
| telephone-Number |phoneNumbers[type eq "work"]type eq "work"].value |
...
| members |members |
| objectId |id |
| proxyAddresses |emails[type eq "other"]type eq "other"].Value |
...
* correlationIdentifier: System.Net.Http.HttpRequestMessage.GetOwinEnvironment["owin.RequestId"]"owin.RequestId"] 
