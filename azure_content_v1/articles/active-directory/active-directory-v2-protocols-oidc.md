...
> [!TIP]
> Try pasting the below into a web browser!
> 
> 
```
...
> [!TIP]
> Try pasting the below request into a browser!
> 
> 
```
...
Once you've gotten an authorization `code` and an `id_token`, you can sign the user in and get access tokens on their behalf.  To sign the user in, you must validate the `id_token` exactly as described [above](#validating-the-id-token.md).  To get access tokens, you can follow the steps described in our [OAuth protocol documentation](active-directory-v2-protocols-oidc.md#request-an-access-token).
