...
* In the TodoListClient project, open `app.config`.  Replace the values of the elements in the `<appSettings>` section to reflect the values you input into the app registration portal.  Your code will reference these values whenever it uses ADAL.  * The `ida:ClientId` is the **Application Id** of your app you copied from the portal.
* The `ida:RedirectUri` is the **Redirect Uri** from the portal.


* In the TodoList-Service project, open `web.config` in the root of the project.    * Replace the `ida:Audience` value with the same **Application Id** from the portal.
