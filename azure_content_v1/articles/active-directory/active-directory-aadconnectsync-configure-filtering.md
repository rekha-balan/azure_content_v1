...
Before you start making changes to filtering, make sure you [disable the scheduled task](#disable-scheduled-task.md) so you do not accidently export changes which you have not yet verified to be correct.

Since filtering can remove very many objects at the same time, you want to make sure your new filters are correct before you start exporting any changes to Azure AD. After you have completed the configuration steps, it is strongly recommended you follow the [verification steps](#apply-and-verify-changes.md) before you export and make changes to Azure AD.
...
* [**Domain-based**](#domain-based-filtering.md): This option enables you to select which domains will synchronize to Azure AD. It also allows you to add and remove domains from the sync engine configuration if you make changes to your on-premises infrastructure after you installed Azure AD Connect sync.

* [**Organizational-Unit–based**](#organizational-unitbased-filtering.md):  This filtering option enables you to select which OUs will synchronize to Azure AD. This option will be on all object types in selected OUs.

* [**Attribute–based**](#attribute-based-filtering.md): This option allows you to filter objects based on attribute values on the objects. You can also have different filters for different object types.
...
* [Select the domains](#select-domains-to-be-synchronized.md) which should be included in the synchronization.
* For each added and removed domain, adjust the [run profiles](#update-run-profiles.md).
* [Apply and verify changes](#apply-and-verify-changes.md).
...
7. Continue to adjust the [run profiles](#update-run-profiles.md).
...
* To complete the configuration, [Apply and verify changes](#apply-and-verify-changes.md).
...
![OU](./media/active-directory-aadconnectsync-configure-filtering/ou.png)     * The **Computers** container should be selected for your Windows 10 computers to be successfully synchronized to Azure AD. If your domain joined computers are located in other OUs, make sure those are selected.
...
8. To complete the configuration, [Apply and verify changes](#apply-and-verify-changes.md).
...
![Inbound 3 transformation](./media/active-directory-aadconnectsync-configure-filtering/inbound3.png)  
12. To complete the configuration, [Apply and verify changes](#apply-and-verify-changes.md).
...
8. Click **Save**.
9. To complete the configuration, [Apply and verify changes](#apply-and-verify-changes.md).
