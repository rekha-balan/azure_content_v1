...
> [AZURE.VIDEO getting-started-with-sencha-touch]AZURE.VIDEO getting-started-with-sencha-touch]
> 
> 
...
## Requirements
* Download and install [Sencha Touch](http://wwww.sencha.com/products/touch/download target=_blank.md).

* Download and install [Sencha Cmd Tool](http://www.sencha.com/products/sencha-cmd/download target=_blank.md).
...
2. In the **Configure** tab, verify that `localhost` is already listed in the **Allow requests from host names** list under **Cross-Origin Resource Sharing (CORS)**. If it's not, type `localhost` in the **Host name** field and then click **Save**.
...
1. Require the Azure class files:
