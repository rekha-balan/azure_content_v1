...
In the next tutorial, [Service-side authorization of Mobile Services users](../mobile-services-dotnet-backend-android-authorize-users-in-scripts.md), you will take the user ID value provided by Mobile Services based on an authenticated user and use it to filter the data returned by Mobile Services.
