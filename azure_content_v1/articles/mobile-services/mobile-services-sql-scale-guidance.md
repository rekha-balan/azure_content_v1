...
1. [Diagnosing Problems](#Diagnosing.md)
2. [Indexing](#Indexing.md)
3. [Schema Design](#Schema.md)
4. [Query Design](#Query.md)
5. [Service Architecture](#Architecture.md)
6. [Advanced Troubleshooting](#Advanced.md)
...
For more information on diagnosing SQL issues, see [Advanced Diagnostics](#AdvancedDiagnosing.md) at the bottom of this document.
...
So, if you are frequently doing WHERE or JOIN statements on particular columns, you should make sure you index them. See the section [Creating Indexes](#CreatingIndexes.md) for more information.
...
For more information on indexes, see [Index Annotations in Entity Framework](http://msdn.microsoft.com/data/jj591583.aspx#Index). For further tips on optimizing indexes, see [Advanced Indexing](#AdvancedIndexing.md) at the bottom of this document.
...
For more information on improving query design, including how to analyze query plans, see [Advanced Query Design](#AdvancedQuery.md) at the bottom of this document.
...
2. Configure the connection with the following values   * Server: *should be pre-set to the right value*
...
* Return a range of values by using operators such as BETWEEN, >, >=, <, and <=.  * After the row with the first value is found by using the clustered index, rows with subsequent indexed values are guaranteed to be physically adjacent.


* Use JOIN clauses; typically these are foreign key columns.
* Use ORDER BY, or GROUP BY clauses.  * An index on the columns specified in the ORDER BY or GROUP BY clause may remove the need for the Database Engine to sort the data, because the rows are already sorted. This improves query performance.
...
For more information, see [Monitoring SQL Database Using Dynamic Management Views](http://go.microsoft.com/fwlink/p/?linkid=309725clcid=0x409) and [Missing Index Dynamic Management Views](http://technet.microsoft.com/library/ms345421.aspx).
...
For more information, see [Monitoring SQL Database Using Dynamic Management Views](http://go.microsoft.com/fwlink/p/?linkid=309725clcid=0x409). In addition to executing the query, the **SQL Database Management Portal** gives you a nice shortcut to see this data, by selecting **Summary** for your database and then selecting **Query Performance**:
...
* [Azure SQL Database performance and scaling](http://go.microsoft.com/fwlink/p/?linkid=397217clcid=0x409)
