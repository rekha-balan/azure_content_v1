...
from Node.js applications. The scenarios covered include **sending notifications to Android, iOS, Windows Phone and Windows Store applications**. For more information on notification hubs, see the [Next Steps](#next.md) section.
...
Create a blank Node.js application. For instructions creating a Node.js application, see [Create and deploy a Node.js application to Azure Web Site](/develop/nodejs/tutorials/create-a-website-(mac)/), [Node.js Cloud Service](../cloud-services-nodejs-develop-deploy-app.md) (using Windows PowerShell), or [Web Site with WebMatrix](/develop/nodejs/tutorials/web-site-with-webmatrix/).
...
The connection **connectionstring** value can be obtained from the [Azure Classic Portal](http://manage.windowsazure.com) by performing the following steps:

1. From the [Azure Classic Portal](http://manage.windowsazure.com), select **Service Bus**, and then select the namespace that contains the notification hub.
...
* See the MSDN Reference: [Azure Notification Hubs][]Azure Notification Hubs][]].
