...
While all of the above methods can be used with SQL Data Warehouse, PolyBase's ability to transparently parallelize loads from Azure Blob Storage will make it the fastest tool for loading data.  Check out more about how to [load with PolyBase](sql-data-warehouse-get-started-load-with-polybase.md).  In addition, as many of our users are looking at initial loads in the 100s of Gigabytes to the 10s of Terabytes from on-premise sources, in the below sections we provide some guidance on initial data loading.  
...
> [!NOTE]
> Before moving data from a system such as SQL Server, we suggest reviewing the [Migrate schema](sql-data-warehouse-migrate-schema.md) and [Migrate code](sql-data-warehouse-migrate-code.md) articles of our documentation. 
> 
> 
## Exporting files with BCP
In order to prep your files for movement to Azure, you will need to export them to flat files.  This is best done using the BCP Command-line Utility.  If you do not have the utility yet, it can be downloaded with the [Microsoft Command Line Utilities for SQL Server](http://www.microsoft.com/en-us/download/details.aspx?id=36433).  A sample BCP command might look like the following:
...
If you are moving data in the 5-10 terabyte range or above, we recommend that you use our disk shipping service [Import/Export](https://azure.microsoft.com/en-us/documentation/articles/storage-import-export-service/) in order to accomplish the move.  However, in our studies, we have been able to move data in the single digit TB range comfortably using public internet with AZCopy.  This process can also be sped up or extended with ExpressRoute.

The following steps will detail out how to move data from on-premise into an Azure Storage account using AZCopy.  If you don't have an Azure Storage account in the same region you can create one by following the [Azure Storage Documentation](https://azure.microsoft.com/en-us/documentation/articles/storage-create-storage-account/).  You can also load data from a storage account in a different region, but the performance in this case will not be optimal.  

> [!NOTE]
> This documentation assumes that you have installed the AZCopy command line utility and are able to run it with Powershell.  If this is not the case, then please follow the [AZCopy Installation Instructions](https://azure.microsoft.com/en-us/documentation/articles/storage-use-azcopy/).  
> 
> 
...
* **Express Route**: As stated above, this process can be sped up if express route is enabled.  An overview of Express Route and steps to configure can be found in the [ExpressRoute documentation](http://azure.microsoft.com/en-us/documentation/services/expressroute/). 
...
1. Load data with a 'CREATE TABLE...AS SELECT' statement. 
