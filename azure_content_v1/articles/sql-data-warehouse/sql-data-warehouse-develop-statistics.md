...
For more details, see [DBCC SHOW_STATISTICS](https://msdn.microsoft.com/library/ms174384.aspx) on MSDN.
...
> [!NOTE]
> If you use SQL Server, you might depend on SQL Server to create and update single-column statistics for you as needed. SQL Data Warehouse is different in this aspect. Since the data is distributed, SQL Data Warehouse doesn't automatically aggregate statistics across all the distributed data. It will only generate the aggregated statistics when you create and update statistics.
> 
> 
...
As you understand how you want to query your data you might want to refine this model - especially when the tables are wide. Please refer to the [Implementing statistics management](## Implementing statistics management.md) section for a more advanced method approach.
...
> [!NOTE]
> For more details on [ascending key]ascending key] please refer to the SQL Server 2014 cardinality estimation model whitepaper.
> 
> 
...
SQL Data Warehouse does not have a system stored procedure equivalent to [sp_create_stats][]sp_create_stats][]] in SQL Server. This stored procedure creates a single column statistics object on every column of the database that doesn't already have statistics.
...
> [!NOTE]
> When updating all statistics on a table, SQL Data Warehouse does a scan to sample the table for each statistics. If the table is large, has many columns, and many statistics, it might be more efficient to udpate individual statistics based on need.
> 
> 
For an implementation of an `UPDATE STATISTICS` procedure please see the [temporary tables](./sql-data-warehouse-develop-temporary-tables.md) article. The implementation method is slightly different to the `CREATE STATISTICS` procedure above but the end result is the same.
...
This view brings columns that relate to statistics, and results from the [STATS_DATE()][]STATS_DATE()][]]function together.
