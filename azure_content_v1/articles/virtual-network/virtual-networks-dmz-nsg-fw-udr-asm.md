...
> [!NOTE]
> There is a limitation using User Defined Routing (UDR) and ExpressRoute due to the complexity of dynamic routing used on the Azure Virtual Gateway. Subnets communicating to the Azure Gateway providing the ExpressRoute connection should not have UDR applied. Also, the Azure Gateway cannot be the NextHop device for other UDR bound subnets. The ability to fully integrate UDR and ExpressRoute will be enabled in a future Azure release.
> 
> 
...
2. Once the route table is created, specific user defined routes can be added. In this snipped, all traffic (0.0.0.0/0) will be routed through the virtual appliance (a variable, $VMIP[0]0], is used to pass in the IP address assigned when the virtual appliance was created earlier in the script). In the script, a corresponding rule is also created in the Frontend table.
...
> 
> 
An endpoint can be opened either at the time of VM creation or post build as is done in the example script and shown below in this code snippet (Note; any item beginning with a dollar sign (e.g.: $VMName[$i]$i]) is a user defined variable from the script in the reference section of this document. The “$i” in brackets, [$i]$i], represents the array number of a specific VM in an array of VMs):
...
5. Firewall begins rule processing:   1. FW Rule 1 (FW Mgmt) doesn’t apply, move to next rule
...
6. The Frontend subnet begins inbound rule processing:   1. NSG Rule 1 (Block Internet) doesn’t apply (this traffic was NAT’d by the firewall, thus the source address is now the firewall which is on the Security subnet and seen by the Frontend subnet NSG to be “local” traffic and is thus allowed), move to next rule
...
11. Firewall begins rule processing:    1. FW Rule 1 (FW Mgmt) doesn’t apply, move to next rule
...
12. The Backend subnet begins inbound rule processing:    1. NSG Rule 1 (Block Internet) doesn’t apply, move to next rule
2. Default NSG Rules allow subnet to subnet traffic, traffic is allowed, stop NSG rule processing
...
17. Frontend subnet begins inbound rule processing:    1. NSG Rule 1 (Block Internet) doesn’t apply, move to next rule
2. Default NSG Rules allow subnet to subnet traffic, traffic is allowed, stop NSG rule processing
...
3. No NSG assigned to Security subnet, so system NSG rules allow traffic to firewall
4. Firewall begins rule processing:   1. FW Rule 1 (FW Mgmt) doesn’t apply, move to next rule
...
5. The Backend subnet begins inbound rule processing:   1. NSG Rule 1 (Block Internet) doesn’t apply (this traffic was NAT’d by the firewall, thus the source address is now the firewall which is on the Security subnet and seen by the Backend subnet NSG to be “local” traffic and is thus allowed), move to next rule
2. Default NSG Rules allow subnet to subnet traffic, traffic is allowed, stop NSG rule processing
...
6. The Backend subnet begins inbound rule processing:   1. NSG Rule 1 (Block Internet) doesn’t apply, move to next rule
2. Default NSG Rules allow subnet to subnet traffic, traffic is allowed, stop NSG rule processing
...
14. The Backend subnet begins inbound rule processing:    1. NSG Rule 1 (Block Internet) doesn’t apply, move to next rule
2. Default NSG Rules allow subnet to subnet traffic, traffic is allowed, stop NSG rule processing
...
19. Frontend subnet begins inbound rule processing:    1. There is no NSG rule that applies to Inbound traffic from the Backend subnet to the Frontend subnet, so none of the NSG rules apply
...
5. Frontend subnet begins inbound rule processing:   1. NSG Rule 1 (Block Internet) doesn’t apply, move to next rule
2. Default NSG Rules allow subnet to subnet traffic, traffic is allowed, stop NSG rule processing
...
10. Backend subnet begins inbound rule processing:    1. NSG Rule 1 (Block Internet) doesn’t apply, move to next rule
2. Default NSG Rules allow subnet to subnet traffic, traffic is allowed, stop NSG rule processing
...
3. If SQL endpoints were open for some reason, the firewall would begin rule processing:   1. FW Rule 1 (FW Mgmt) doesn’t apply, move to next rule
2. FW Rules 2 - 5 (RDP Rules) don’t apply, move to next rule
...
5. Configure UDR including:   1. Creating two new route tables
2. Add routes to the tables
3. Bind tables to appropriate subnets


6. Enable IP Forwarding on the NVA
7. Configure NSG including:   1. Creating a NSG
2. Adding a rule
